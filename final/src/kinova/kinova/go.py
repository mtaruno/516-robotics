#!/usr/bin/env python3
"""
Pick-and-Place Routine with TurtleBot Movement

This script demonstrates a pick-and-place routine for a Kinova Gen3 Lite robot arm
and gripper, coordinated with TurtleBot base movement. The routine consists of:
  1. Moving the TurtleBot forward to a target location.
  2. Executing a pick-and-place operation with the robot arm and gripper.
  3. Moving the TurtleBot backward to its starting position.

Note:
  - This example uses ROS2 and MoveIt2 interfaces.
  - Adjust the pose parameters as needed for your environment.


IP for cable connection for middle bot: 192.168.1.10
...
def execute(robot_arm, gripper_control,
                             approach_position, grasp_position,
                             transit_position1, transit_position2,
                             release_position1, release_position2, release_position3):
    """
    Executes a pick-and-place transfer operation using the provided arm and gripper.
    
    The sequence is:
      1. Move to the approach position.
      2. Open the gripper.
      3. Move to the grasp position and close the gripper.
      4. Move through two transit positions.
      5. Move through three release positions.
    ...
    # gripper_control.move_to_position(0.0)  # (Alternative gripper command)
    gripper_control.open()

    # Step 3: Move to grasp position and close the gripper.
    robot_arm.inverse_kinematic_movement(grasp_position)
    # gripper_control.move_to_position(0.8)  # (Alternative gripper command)
    gripper_control.close()
    print("got the cube")
    time.sleep(0.5)

    # Steps 4 & 5: Transit to the release positions.
...
def create_robot_pose(position_x, position_y, position_z,
                      orientation_x, orientation_y, orientation_z, orientation_w):
    """
    Creates and returns a Pose message with the specified position and orientation.
    
    Args:
      position_x, position_y, position_z: Cartesian coordinates.
      orientation_x, orientation_y, orientation_z, orientation_w: Quaternion components.
...
class TurtleBotController(Node):
    """
    A simple controller for the TurtleBot that publishes Twist messages to control movement.
    """
    def __init__(self, speed=0.1, distance=1.0):
        """
        Initializes the TurtleBot controller.
        
        Args:
          speed (float): Linear speed (positive for forward, negative for reverse).
          distance (float): The target distance to move.
        """
    ...
    def move(self):
        """
        Moves the TurtleBot for a specified distance while displaying progress.
        """
        ...
        # Calculate movement duration and initialize distance tracking.
        duration = self.target_distance / abs(self.speed)
        start_time = time.time()
        traveled_distance = 0.0

        # Publish the movement command until the target distance is reached.
...
def main():
    """
    Main function to initialize ROS2, execute the TurtleBot and robotic arm routines,
    and perform cleanup.
    """
    # Initialize ROS2.
    rclpy.init()
    ...
    # ---------------------------------------------------------
    # Part 1: Move TurtleBot forward to the pick-and-place zone.
    # ---------------------------------------------------------
    turtlebot_forward = TurtleBotController(speed=0.2, distance=3.375)
    turtlebot_forward.move()

    # ---------------------------------------------------------
    # Part 2: Execute the pick-and-place operation.
    # ---------------------------------------------------------
    ...
    approach_position = create_robot_pose(0.370, -0.092, 0.371,
                                          0.817, 0.574, -0.036, 0.028)
    # alt_approach_position = create_robot_pose(0.380, -0.030, 0.371, 0.817, 0.574, -0.036, 0.028)

    grasp_position = create_robot_pose(0.370, -0.070, 0.130,
                                       0.758, 0.652, 0.008, 0.017)
    # alt_grasp_position = create_robot_pose(0.394, -0.031, 0.161, 0.751, 0.659, -0.025, 0.017)

    transit_position1 = create_robot_pose(-0.293, 0.081, 0.228,
                                          0.752, 0.658, 0.023, -0.027)
    transit_position2 = create_robot_pose(-0.174, -0.002, 0.191,
                                          0.494, 0.867, -0.007, -0.066)

    release_position1 = create_robot_pose(-0.195, 0.034, -0.164,
                                          0.494, 0.867, -0.008, -0.066)
    release_position2 = create_robot_pose(-0.173, 0.0, -0.342,
                                          0.542, 0.838, -0.04, -0.042)
    release_position3 = create_robot_pose(-0.159, 0.085, -0.336,
                                          -0.018, 0.997, 0.026, -0.076)
    # Standard movement duration: approximately 2 minutes 5 seconds.

    # Execute the robotic transfer sequence.
    execute(arm, gripper, approach_position, grasp_position, transit_position1, transit_position2, 
                             release_position1, release_position2, release_position3)

    # ---------------------------------------------------------
    # Part 3: Move TurtleBot backward to the starting position.
    # ---------------------------------------------------------
    turtlebot_reverse = TurtleBotController(speed=-0.1, distance=3.2)
    turtlebot_reverse.move()

    # ---------------------------------------------------------
    # Cleanup: Destroy nodes and shutdown ROS2.
    # ---------------------------------------------------------
    turtlebot_forward.destroy_node()
    turtlebot_reverse.destroy_node()
    rclpy.shutdown()
    gripper.shutdown()
    arm.shutdown()
